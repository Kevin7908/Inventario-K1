# share — Biblioteca de Widgets Compartidos
...
Tres preguntas para saber si algo pertenece a `share`:
...
Si las tres respuestas son **sí**, va en `share`.

---

## Regla fundamental

**Todo widget en `share` es `StatelessWidget`.**
...
```
❌ share decide qué mostrar
✓  share recibe qué mostrar y lo pinta
```
...
├── share.dart                      ← barrel raíz: importa todo share desde aquí
│
...
| `PanelSinDatos` | El hermano chico del anterior: el cajón con borde que rellena un `PanelSeccion` de una ficha mientras no hay nada que pintar, o mientras carga (`PanelSinDatos.cargando()`) |
...
Los widgets de `share` **no** usan `Theme.of(context)` directamente. Usan `ColoresApp` y `TipografiaApp` que son las fuentes de verdad del diseño visual.

---

## Importaciones

Se usa un barrel raíz `share.dart` en la raíz de esta carpeta. Es el único punto de importación necesario para cualquier módulo del proyecto.

```dart
// Una sola línea da acceso a todo share
...
```dart
import 'package:inventario_k1/frontend/share/botones/botones.dart';
import 'package:inventario_k1/frontend/share/tablas/tablas.dart';
```
...
Antes de crear un widget en `share`, verificar:
...
2. **¿Pertenece a `share`?** Debe cumplir las tres condiciones de la sección de filosofía: reutilizable, sin lógica de negocio y sin dependencias externas.
...
5. **Agregar al barrel.** El archivo nuevo debe exportarse en el barrel de su categoría (`categoria/categoria.dart`) y este a su vez debe estar en el barrel raíz (`share.dart`).
...
La regla no cambia: **una tarea, un widget**. Que no quepa en `share` no
...
- **Layout de pantalla completa**: la estructura de cada pantalla (qué va arriba, qué va al lado) la decide cada feature, no share.
- **Dependencias de `pub.dev`**: cero paquetes externos. Solo Flutter SDK.

---

## Por qué durante un tiempo se llamó `share2`

Hubo una biblioteca anterior en esta misma ruta. La migración fue módulo a
módulo, y mientras duró convivieron las dos: la vieja congelada en `share/` y
la nueva en `share2/`.

La vieja **se borró el 28/08/2026**, al quedarse sin un solo consumidor, y esta
recuperó el nombre. No queda nada que migrar ni de dónde copiar. Si un commit
viejo o una rama sin mergear importa `share2/`, es esa etapa: la ruta está
muerta.
